...
class SigmoidMetric():
    ...
    def batch_pix_accuracy(self, output, target):
        ...
        predict = (output  > 5e-1).astype('int64') # P
    ...
    def batch_intersection_union(self, output, target):
        ...
        maxi = 1 # nclass
        nbins = 1 # nclass
        predict = (output.cpu().detach().numpy() > 5e-1).astype('int64') # P
...
class SamplewiseSigmoidMetric():
    ...
    def batch_intersection_union(self, output, target, nclass, score_thresh):
        ...
        output = (output > 5e-1)  # P
        predict = (torch.sigmoid(output).cpu().detach().numpy() > score_thresh).astype('int64')
